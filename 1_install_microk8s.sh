...
case "$CMD" in
create)
	echo "Creating Kubernetes Cluster: master ${MEM}G ${VCPU} cpu, $NUM workers with ${VCPU} cpu, ${MEM}G mem, ${DISK}G disk"
	build $NUM "-c ${VCPU} -d ${DISK}G -m ${MEM}G" "-c ${VCPU} -d ${DISK}G -m ${MEM}G"
	config
;;
destroy)
...
*)
	...
	echo "defaults: 2 workers with 2 vcpu with 4G mem 15G disk"
;;
esac
